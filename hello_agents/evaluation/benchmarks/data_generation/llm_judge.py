...
class LLMJudgeEvaluator:
    ...
    def __init__(
        self, llm: Optional[HelloAgentsLLM] = None, judge_model: str = "gpt-4o"
    ):
        """
        初始化LLM Judge评估器
    ...
    def evaluate_batch(
        ...
        for idx, problem in enumerate(problems):
            ...
            reference = (
                references[idx] if references and idx < len(references) else None
            )
            ...
            # 显示评分
            print(
                f"   ✓ {problem.get('problem_id', 'unknown')}: {result['total_score']:.2f}/5.0"
            )
...
"""
    ...
    def _compute_metrics(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "excellent_rate": sum(1 for s in total_scores if s >= 4.5)
            / len(total_scores),
        }
