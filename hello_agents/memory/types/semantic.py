...
class SemanticMemory(BaseMemory):
    ...
    def _init_nlp(self):
        ...
        try:
            ...
            # 尝试加载多语言模型
            models_to_try = [("zh_core_web_sm", "中文"), ("en_core_web_sm", "英文")]
    ...
    def add(self, memory_item: MemoryItem) -> str:
        ...
        try:
            ...
            success = self.vector_store.add_vectors(
                vectors=[embedding.tolist()], metadata=[metadata], ids=[memory_item.id]
            )
            ...
            logger.info(
                f"✅ 添加语义记忆: {len(entities)}个实体, {len(relations)}个关系"
            )
    ...
    def _graph_search(
        ...
        try:
            ...
            if not query_entities:
                ...
                if entities_by_name:
                    query_entities = [
                        Entity(entity_id=e["id"], name=e["name"], entity_type=e["type"])
            ...
            for entity in query_entities:
                try:
                    ...
                    related_entities = self.graph_store.find_related_entities(
                        entity_id=entity.entity_id, max_depth=2, limit=20
                    )
                    ...
                    # 也添加直接匹配的实体记忆
                    entity_rels = self.graph_store.get_entity_relationships(
                        entity.entity_id
                    )
    ...
    def _combine_and_rank_results(
        ...
        # 调试信息
        logger.debug(
            f"🔍 向量结果: {len(vector_results)}, 图结果: {len(graph_results)}"
        )
    ...
    def _extract_entities(self, text: str) -> List[Entity]:
        ...
        logger.debug(
            f"🌐 检测语言: {lang}, 使用模型: {selected_nlp.meta['name'] if selected_nlp else 'None'}"
        )
        ...
        if selected_nlp:
            try:
                ...
                for ent in doc.ents:
                    ...
                    logger.debug(
                        f"🏷️ spaCy识别实体: '{ent.text}' -> {ent.label_} (置信度: {confidence})"
                    )
    ...
    def _calculate_graph_relevance_neo4j(
        self, memory_metadata: Dict[str, Any], query_entities: List[Entity]
    ) -> float:
        """计算Neo4j图相关性分数"""
        try:
            ...
            entity_score = (
                matching_entities / len(query_entity_ids) if query_entity_ids else 0
            )
    ...
    def _find_memory_by_id(self, memory_id: str) -> Optional[MemoryItem]:
        """根据ID查找记忆"""
        logger.debug(
            f"🔍 查找记忆ID: {memory_id}, 当前记忆数: {len(self.semantic_memories)}"
        )
        ...
        for memory in self.semantic_memories:
            ...
            if strategy == "importance_based":
                ...
                if len(self.semantic_memories) > self.config.max_capacity:
                    ...
                    excess_count = (
                        len(self.semantic_memories) - self.config.max_capacity
                    )
    ...
    def smart_forget(self, threshold: float = 0.35, weights: dict = None) -> int:
        ...
        for memory in self.semantic_memories:
            access_count = self.access_counts.get(memory.id, 0)
            last_access_time = self.last_access_times.get(memory.id, memory.timestamp)
    ...
    def get_related_entities(
        self, entity_id: str, relation_types: List[str] = None, max_hops: int = 2
    ) -> List[Dict[str, Any]]:
        """获取相关实体 - 使用Neo4j图数据库"""
    ...
    def export_knowledge_graph(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"entities": {}, "relations": [], "graph_stats": {"error": str(e)}}
