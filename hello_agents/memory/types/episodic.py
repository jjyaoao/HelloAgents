...
class EpisodicMemory(BaseMemory):
    ...
    def retrieve(self, query: str, limit: int = 5, **kwargs) -> List[MemoryItem]:
        ...
        try:
            ...
            hits = self.vector_store.search_similar(
                query_vector=query_vec, limit=max(limit * 5, 20), where=where
            )
        except Exception:
            hits = []
        ...
        for hit in hits:
            hit.access_count += 1
            hit.last_access_time = datetime.now()
    ...
    def update(
        ...
        if content is not None:
            try:
                ...
                self.vector_store.add_vectors(
                    vectors=[embedding], metadata=[payload], ids=[memory_id]
                )
            except Exception:
                pass
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_importance": sum(e.importance for e in active_episodes)
            / len(active_episodes)
            if active_episodes
            else 0.0,
    ...
    def find_patterns(
        self, user_id: str = None, min_frequency: int = 2
    ) -> List[Dict[str, Any]]:
        """发现用户行为模式"""
        # 检查缓存
        ...
        if (
            ...
            and (datetime.now() - self.last_pattern_analysis).hours < 1
        ):
    ...
    def smart_forget(self, threshold: float = 0.35, weights: dict = None) -> int:
        ...
        for episode in self.episodes:
            # 访问频率得分
            access_score = min(1.0, episode.access_count / 10.0)

            # 时间衰减得分
            delta_days = (now - episode.last_access_time).days
