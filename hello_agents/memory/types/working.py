...
class WorkingMemory(BaseMemory):
    ...
    def retrieve(
        self, query: str, limit: int = 5, user_id: str = None, **kwargs
    ) -> List[MemoryItem]:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "token_usage": self.current_tokens / self.max_tokens
    ...
    def smart_forget(self, threshold: float = 0.35, weights: dict = None) -> int:
        ...
        for memory in self.memories:
            access_score = min(1.0, self._calculate_priority(memory) / 1.0)
            delta_days = (now - memory.timestamp).days
