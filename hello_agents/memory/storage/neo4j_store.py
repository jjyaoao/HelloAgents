...
class Neo4jGraphStore:
    ...
    def add_relationship(
        ...
        try:
            ...
            with self.driver.session(database=self.database) as session:
                result = session.run(
                    query, from_id=from_entity_id, to_id=to_entity_id, properties=props
                )
                record = result.single()
    ...
    def search_entities_by_name(
        self, name_pattern: str, entity_types: List[str] = None, limit: int = 20
    ) -> List[Dict[str, Any]]:
        """
        按名称搜索实体
    ...
    def clear_all(self) -> bool:
        ...
        try:
            ...
            with self.driver.session(database=self.database) as session:
                ...
                logger.info(
                    f"✅ 清空Neo4j数据库: 删除 {deleted_nodes} 个节点, {deleted_relationships} 个关系"
                )
                return True
