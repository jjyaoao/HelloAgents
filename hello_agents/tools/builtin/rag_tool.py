...
class RAGTool(Tool):
        ...
        try:
            if action == "add_document":
                ...
                return self._ask(
                    ...
                    enable_advanced_search=parameters.get(
    ...
    @tool_action(
        "rag_add_document",
        "添加文档到知识库（支持PDF、Word、Excel、PPT、图片、音频等多种格式）",
    )
    def _add_document(
        self,
        file_path: str,
    ...
    def _ask(
        ...
        try:
            ...
            if enable_advanced_search:
                results = pipeline["search_advanced"](
                    query=user_question, top_k=limit, enable_mqe=True, enable_hyde=True
                )
            else:
    ...
    def _clear_knowledge_base(
        self, confirm: bool = False, namespace: str = "default"
    ) -> str:
        """清空知识库
            ...
            if stats:
                ...
                if "config" in stats:
                    ...
                    if isinstance(config, dict):
                        ...
                        stats_info.extend(
                            [f"🔢 向量维度: {vector_size}", f"📎 距离度量: {distance}"]
                        )
    ...
    def add_documents_batch(
        self, file_paths: List[str], namespace: str = "default"
    ) -> str:
        """批量添加多个文档"""
        if not file_paths:
            return "❌ 文件路径列表不能为空"
