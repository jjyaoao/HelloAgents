...
class NoteTool(Tool):
    ...
    def run(self, parameters: Dict[str, Any]) -> str:
        ...
        if action == "create":
            ...
            return self._list_notes(
                note_type=parameters.get("note_type"), limit=parameters.get("limit", 10)
            )
        elif action == "search":
        ...
        return [
            ToolParameter(
                ...
                description=(
                    ...
                    "delete(删除), list(列表), search(搜索), summary(摘要), organize(自动整理)"
                ),
                required=True,
            ),
            ToolParameter(
            ...
            ToolParameter(
                name="mode",
                type="string",
                description="整理模式: auto(自动) 或 manual(手动)，仅organize时有效",
                required=False,
                default="auto",
            ),
            ToolParameter(
                name="force",
                type="boolean",
                description="是否强制执行整理（忽略阈值检查），仅organize时有效",
                required=False,
                default=False,
            ),
        ]
    ...
    def _organize_notes(self, mode: str = "auto", force: bool = False) -> str:
        ...
        自动分析临时笔记，将重要信息升级为任务笔记或项目笔记，
        清理冗余、过时、重复的内容。

        Args:
            mode: 整理模式，"auto"自动模式或"manual"手动模式
            force: 是否强制执行（忽略阈值检查）

        Returns:
            整理结果
        """
        from .note_organizer import NoteOrganizer, NoteOrganizerPolicy

        policy = NoteOrganizerPolicy(
            temp_notes_threshold=10,
            auto_promote_enabled=(mode == "auto"),
            auto_cleanup_enabled=(mode == "auto"),
            auto_merge_enabled=(mode == "auto"),
        )
