...
# 2. 创建LLM Judge评估器
llm = HelloAgentsLLM(provider="deepseek", model_name="deepseek-chat")
...
for i, problem in enumerate(generated_problems, 1):
    ...
    # 显示评估结果
    print(f"\n评估结果:")
    print(f"  正确性: {result['scores']['correctness']}/5")
    print(f"  清晰度: {result['scores']['clarity']}/5")
    print(f"  难度匹配: {result['scores']['difficulty_match']}/5")
    print(f"  完整性: {result['scores']['completeness']}/5")
    ...
    try:
        ...
        print("  [评估文本包含特殊字符，无法在控制台显示]")
        print(f"  文本长度: {len(result['evaluation_text'])} 字符")
